...
export class TwitterScraper {
  ...
  async #fetchProfile(page, profile) {
    ...
    for (const group of ranked) {
      ...
      // Prefer DOM/time timestamps; snowflake id encodes create time as backup.
      const leaf =
  ...
  async #fetchStatusThread(page, statusUrl, authorHandle) {
    ...
    // Expand truncated tweet text only — never click generic "Show more" UI
    // (sidebar/explore links), which navigates away from the status page.
    await page.evaluate(() => {
    ...
    // If a stray click or soft-nav left the status page, bail with empty result.
