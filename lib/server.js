...
export function startServer(cfg, stores = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (path === "/facebook.xml" || path === "/feed.xml" || path === "/") {
      return serveFeed(res, cfg.facebook?.feed || cfg.feed, stores.facebook, "facebook");
    }
  ...
  return new Promise((resolve, reject) => {
    server.listen(port, () => {
      ...
      console.log(`feeds: Legacy alias http://localhost:${port}/feed.xml → facebook`);
      resolve(server);
    });
